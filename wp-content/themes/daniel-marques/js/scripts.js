(function($){
	...
	function initPostShare() {
	    "use strict";
	    var i = $(".entry-share > .popup");
	    $("body").on("click", i.selector, function(i) {
	        i.preventDefault(), openPopup($(this).attr("href"), "", 500, 300)
	    })
	}
	...
	$(document).ready(function(){
		...
		$("#search_button:not(.has-input), #Top_bar .icon_close").click(function(e){
			e.preventDefault();
			if( $('#Top_bar .search_wrapper').hasClass('open') ){
				$('#Top_bar .search_wrapper').removeClass('open');
				$('#Top_bar .search_wrapper').css({
					display: 'none',
					opacity: 0
				});
			} else{
				$('#Top_bar .search_wrapper').addClass('open');
				$('#Top_bar .search_wrapper').css({
					display: 'block',
					opacity: 1
				});
