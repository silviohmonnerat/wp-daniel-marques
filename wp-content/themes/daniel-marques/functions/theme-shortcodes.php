...
if( ! function_exists( 'sc_defense' ) ){
	function sc_defense( $attr, $content = null ){
		extract(shortcode_atts(array(
			'title'      => '',
			'post_type'  => '',
			'count'      => 6,
		...
		$output = '<div clasd="column one column_blog">';
			$output .= '<div clasd="blog_wrapper isotope_wrapper">';
				$output .= '<div clasd="posts_group lm_wrapper classic col-3">';

				while ( $projetos->have_posts() ) : $projetos->the_post();

					$output .= '<article class="post-item isotope-item clearfix post-item-denuncia" id="'.get_the_ID().'">';
						...
						$output .= '<div class="image_frame post-photo-wrapper scale-with-grid '. mfn_post_thumbnail_type( get_the_ID() ) .'">';
							$output .= '<div class="image_wrapper">';
								$output .= mfn_post_thumbnail( get_the_ID(), 'blog', false, false  );
							$output .= '</div>';
						$output .= '</div>';
						...
						// content -------------------------------------
						$output .= '<div class="post-excerpt" itemprop="summary">'. get_the_content() .'</div>';

						// Action link ------------------------------------
						$output .= '<div class="bt-action-post defesa">';
							$output .= '<div class="entry-share">
								<a class="popup facebook" href="'.esc_url( 'https://www.facebook.com/sharer/sharer.php?u=' . get_the_permalink() ).'"><i class="icon-facebook"></i></a>
								<a class="popup twitter" href="'.esc_url( 'http://twitter.com/share?text=' . urlencode( get_the_title() ) . '&amp;url=' . urlencode( get_the_permalink() ) ).'"><i class="icon-twitter"></i></a>
								<a class="popup pinterest" href="'.esc_url( 'http://pinterest.com/pin/create/button/?url=' . urlencode( get_the_permalink() ) . '&amp;media=' . urlencode( get_the_permalink() ) . '&amp;description=' . urlencode( get_the_excerpt() ) ).'"><i class="icon-gplus"></i></a>
								<a class="popup googleplus" href="'.esc_url( 'https://plus.google.com/share?url=' . get_the_permalink() ).'"><i class="icon-pinterest"></i></a>
							</div>';
							if(get_post_meta( get_the_ID(), 'mfn-defesa-pdf', true )) :
							$output .= '<a class="readmore" href="'.get_post_meta( get_the_ID(), 'mfn-defesa-pdf', true ).'" target="_blank">Saiba mais <i class="icon-link" style="transition: none; line-height: 24px; border-width: 0px; margin: 0px; padding: 0px; letter-spacing: 0px; font-size: 20px;"></i> </a>';
							endif;
						$output .= '</div>';
				...
				$output .= '</div>';
				// pagination
				if( $pagination ) $output .= mfn_pagination( $projetos, $load_more );
...
if( ! function_exists( 'sc_project_laws' ) ){
	function sc_project_laws( $attr, $content = null ){
		extract(shortcode_atts(array(
			'title'      => '',
			'post_type'  => '',
			'count'      => 6,
		...
		$output = '<div clasd="column one column_blog">';
			$output .= '<div clasd="blog_wrapper isotope_wrapper">';
				...
				while ( $projetos->have_posts() ) : $projetos->the_post();
					...
					$output .= '<article class="post-item isotope-item clearfix post-item-projeto" id="'.get_the_ID().'">';
						...
						// Action link ------------------------------------
						$output .= '<div class="bt-action-post">';
							$output .= '<div class="entry-share">
								<a class="popup facebook" href="'.esc_url( 'https://www.facebook.com/sharer/sharer.php?u=' . get_the_permalink() ).'"><i class="icon-facebook"></i></a>
								<a class="popup twitter" href="'.esc_url( 'http://twitter.com/share?text=' . urlencode( get_the_title() ) . '&amp;url=' . urlencode( get_the_permalink() ) ).'"><i class="icon-twitter"></i></a>
								<a class="popup pinterest" href="'.esc_url( 'http://pinterest.com/pin/create/button/?url=' . urlencode( get_the_permalink() ) . '&amp;media=' . urlencode( get_the_permalink() ) . '&amp;description=' . urlencode( get_the_excerpt() ) ).'"><i class="icon-gplus"></i></a>
							...
							if(get_post_meta( get_the_ID(), 'mfn-projeto-lei-pdf', true )) :
							$output .= '<a class="readmore" href="'.get_post_meta( get_the_ID(), 'mfn-projeto-lei-pdf', true ).'" target="_blank">Veja o Projeto de Lei <i class="icon-link" style="transition: none; line-height: 24px; border-width: 0px; margin: 0px; padding: 0px; letter-spacing: 0px; font-size: 20px;"></i> </a>';
							endif;
						$output .= '</div>';
...
if( ! function_exists( 'sc_video_vimeo' ) ){
	function sc_video_vimeo( $attr, $content = null ){
		extract(shortcode_atts(array(
			'title'        => '',
			'user_id'      => '',
			'access_token' => '',
		...
		$json    = wp_remote_get('https://api.vimeo.com/users/'.$user_id.'/videos?access_token='.$access_token.'&sort=date&direction=desc');
		$result  = wp_remote_retrieve_body( $json );
		$resJson = json_decode( $result );
		$id = 1;
					
		$output = '<div clasd="column one">';
			$output .= '<div class="inner-content-video">';

				foreach ($resJson->data as $data) {
					$output .=  '
					<div class="box-vvideo vc_col-sm-4">
						<h3>'.$data->name.'</h3>
						<a class="prettyphoto" href="'.$data->link.'">
							<img src="'.$data->pictures->sizes[3]->link_with_play_button.'">
						</a>
					</div>';
					$id++;
				}
...
if( ! function_exists( 'sc_video_facebook' ) ){
	function sc_video_facebook($atts, $content = null) {
	   extract(shortcode_atts(array(
	      "v_id" => '',
	   ), $atts));
	   ...
	   <param name="movie" value="http://www.facebook.com/v/'.$v_id.'" />
	   <embed src="http://www.facebook.com/v/'.$v_id.'" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="400" height="224">
	   </embed>
	   </object>';
	}
}
...
// column shortcodes -----------------------
add_shortcode( 'defense', 'sc_defense' );
add_shortcode( 'laws', 'sc_project_laws' );
add_shortcode( 'vvideo', 'sc_video_vimeo' );
add_shortcode( 'fbvideo', 'sc_video_facebook');
